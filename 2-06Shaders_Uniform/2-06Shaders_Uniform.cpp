// 2-06Shaders_Uniform : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// Uniform是一种从CPU中的应用向GPU中的着色器发送数据的方式,使用uniform向着色器动态传入数据
...
int main()
{
    ...
    // glfw: 创建窗口
    // ------------------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-06Shaders_Uniform", NULL, NULL);
    if (window == NULL)
    {
        printf("窗口创建失败");
...
#pragma region 构建和编shader程序
    ...
    shaderProgram1 = glCreateProgram();//构建一个着色器程序
    glAttachShader(shaderProgram1, vertexShader);
    glAttachShader(shaderProgram1, fragmentShader1);
    glLinkProgram(shaderProgram1);//链接着色器程序供给opgl使用
    ...
    shaderProgram2 = glCreateProgram();//构建一个着色器程序
    glAttachShader(shaderProgram2, vertexShader);
    glAttachShader(shaderProgram2, fragmentShader2);
    glLinkProgram(shaderProgram2);//链接着色器程序供给opgl使用
...
#pragma region 处理渲染数据，绘制指令等
    ...
    glGenVertexArrays(2, VAOs);//glGenVertexArrays用于创建一个VAO
    glGenBuffers(2, VBOs);//glBufferData用于创建一个顶点缓冲对象
    ...
    //绑定VAO VBO
    // -----------------------------
    glBindVertexArray(VAOs[0]);//glBindVertexArray用于绑定一个VAO
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[0]);//glBindBuffer用于绑定一个顶点缓冲对象
    ...
    // -----------------------------
    glBufferData(GL_ARRAY_BUFFER, sizeof(firstTriangle), firstTriangle, GL_STATIC_DRAW);//glBufferData用于配置VBO
    ...
    //绑定VAO VBO
    // -----------------------------
    glBindVertexArray(VAOs[1]);//glBindVertexArray用于绑定一个VAO
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[1]);//glBindBuffer用于绑定一个顶点缓冲对象
    ...
    //我们可以通过glVertexAttribPointer与glBufferData来分别配置VAO和VBO
    // -----------------------------
    glBufferData(GL_ARRAY_BUFFER, sizeof(secondTriangle), secondTriangle, GL_STATIC_DRAW);//glBufferData用于配置VBO
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);//glVertexAttribPointer用于配置VAO
    glEnableVertexAttribArray(0);//glEnableVertexAttribArray用来启用VAO的槽位
