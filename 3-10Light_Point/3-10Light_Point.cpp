// 3-10Light_Point : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
衰减
随着光线传播距离的增长逐渐削减光的强度通常叫做衰减(Attenuation)。随距离减少光强度的一种方式是使用一个线性方程。
这样的方程能够随着距离的增长线性地减少光的强度，从而让远处的物体更暗。
然而，这样的线性方程通常会看起来比较假。在现实世界中，灯在近处通常会非常亮，
但随着距离的增加光源的亮度一开始会下降非常快，但在远处时剩余的光强度就会下降的非常缓慢了。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3-10Light_Point", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
...
#pragma region Draw
		...
		lightingShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);s
		//点光源衰减
