// 6-16SSAO : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
其中一种间接光照的模拟叫做屏幕空间环境光遮蔽(Screen-Space Ambient Occlusion, SSAO)的技术。
SSAO是一种用于模拟环境光遮蔽效果的渲染技术，通过在屏幕空间内对场景中的几何信息进行采样和分析，
生成一种模拟遮蔽的效果。它主要用于增强图形渲染的真实感和细节。

SSAO的基本原理是在屏幕空间内对每个像素的深度信息进行采样，并通过随机采样周围的像素来计算每个像素的遮蔽程度。这种遮蔽程度可以通过计算周围像素深度与当前像素深度之间的距离来获得，距离越近的像素遮蔽程度越高。

SSAO通常分为以下几个步骤：

1.深度提取：将场景中每个像素的深度信息提取到一个深度缓冲区中，这通常是在延迟着色器中完成的。

2.随机采样：在屏幕空间内随机采样一组像素，并计算每个像素与当前像素之间的距离。

3.遮蔽计算：根据距离和一些参数（如采样半径和遮蔽强度）计算每个像素的遮蔽程度，并将其存储到一个纹理中。

4.合成：将遮蔽纹理与场景中的颜色纹理进行合成，生成最终的渲染图像。

SSAO的优点是它可以增强图形渲染的真实感和细节，特别是在室内和暗场景中。
SSAO的缺点是它可能会产生一些不自然的阴影效果，并且对性能有一定的影响。
为了解决这些问题，可以使用一些改进的技术，如HBAO（Horizon-Based Ambient Occlusion）和SSDO（Screen Space Directional Occlusion）等。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "6-16SSAO", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
...
#pragma region 帧缓冲配置
	...
	//GL_CLAMP_TO_EDGE的纹理封装方法。这保证了我们不会不小心采样到在屏幕空间中纹理默认坐标区域之外的深度值。
	...
	// SSAO缓冲,因为环境遮蔽的结果是一个灰度值，我们将只需要纹理的红色分量，所以我们将颜色缓冲的内部格式设置为GL_RED。
	...
	// 半球采样核心
	// ----------------------
	std::uniform_real_distribution<GLfloat> randomFloats(0.0, 1.0); // 生成0.0和1.0之间的随机浮点数
	std::default_random_engine generator;
	std::vector<glm::vec3> ssaoKernel;
	for (unsigned int i = 0; i < 64; ++i)//假设在一个单位半球，获得一个拥有最大64样本值的采样核心：
	{
		...
		// 所有的样本都是平均分布在采样核心里的，但是我们更愿意将更多的注意放在靠近真正片段的遮蔽上，也就是将核心样本靠近原点分布。
		// 我们可以用一个加速插值函数实现ourLerp
	...
	for (GLuint i = 0; i < 16; i++)//创建一个4x4朝向切线空间平面法线的随机旋转向量数组
	{
		glm::vec3 noise(
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);//封装方法为GL_REPEAT，从而保证它合适地平铺在屏幕上。
...
#pragma region Draw
		...
		shaderGeometryPass.setInt("invertedNormals", 1); // 在立方体内部翻转法线
		renderCube();
		...
		// 4.光照通道:通过使用gbuffer的内容在一个填充了四像素的屏幕上迭代计算光照。
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shaderLightingPass.use();
		// 发送相关参数
