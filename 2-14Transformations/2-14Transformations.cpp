// 2-14Transformations.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// 
// 利用矩阵的变换来对所有顶点进行变换
// 
// 矩阵与向量的运算->将顶点数据与一个变换矩阵相乘（可以影响顶点的显示效果，达到旋转，缩放，移动的效果）
// 
// GLM库：OpenGL没有自带任何的矩阵和向量知识，幸运的是，有个易于使用，专门为OpenGL量身定做的数学库，那就是GLM。
//		  GLM是OpenGL Mathematics的缩写，它是一个只有头文件的库，也就是说我们只需包含对应的头文件就行了，不用链接和编译。
// 平移矩阵： glm::translate  把单位矩阵和一个位移向量传递给glm::translate可以得到一个平移矩阵过后的矩阵
// 旋转矩阵： glm::rotate 将需要变换矩阵与（旋转角度（弧度制）+旋转轴（向量））传递给glm::rotate可以得到旋转过后的矩阵，注意：GLM希望它的角度是弧度制的(Radian)，所以我们使用glm::radians将角度转化为弧度
// 缩放矩阵： glm::scale 将需要变换矩阵和一个缩放向量传递给glm::scale可以得到一个缩放过后的矩阵
#include <iostream>
...
#pragma region Draw
		...
		glUseProgram(ourShader.ID);
		//将变换好的矩阵传递给shader  glUniformMatrix4fv可以将一个矩阵传递给shader(渲染管线)
