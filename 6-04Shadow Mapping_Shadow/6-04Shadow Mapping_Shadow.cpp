// 6-04Shadow Mapping_Shadow : 此文件包含 "main" 函数。程序执行将在此处开始并结束。

/*
阴影映射_显示阴影

应用：通过光空间视角的深度投影矩阵贴图(0-1的灰度图)结合当前顶点位置（光空间投影下的）Z轴进行比较来计算是否在阴影中

要检查一个片段是否在阴影中，把光空间片段位置转换为裁切空间的标准化设备坐标。
...
有了这些投影坐标，我们就能从深度贴图中采样得到0到1的结果，从第一个渲染阶段的projCoords坐标直接对应于变换过的NDC坐标。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "6-04Shadow Mapping_Shadow", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
