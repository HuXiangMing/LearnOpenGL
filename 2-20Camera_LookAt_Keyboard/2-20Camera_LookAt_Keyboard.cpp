// 2-20Camera_LookAt_Keyboard : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
 
/*

自由移动（通过键盘按键控制摄像机的位置）

定义摄像机参数变量，传递给lookat，通过检测按键按下改变摄像机变量的值
glm::vec3 cameraPos   = glm::vec3(0.0f, 0.0f,  3.0f);
glm::vec3 cameraFront = glm::vec3(0.0f, 0.0f, -1.0f);//看向-z
glm::vec3 cameraUp    = glm::vec3(0.0f, 1.0f,  0.0f);

view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
cameraPos + cameraFront 可以保证无论我们怎么移动，摄像机都会注视着目标方向
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-20Camera_LookAt_Keyboard", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
