// 2-11Textures_Exercise1 : 此文件包含 "main" 函数。程序执行将在此处开始并结束。// 
// 
// 尝试用不同的纹理环绕方式，设定一个从0.0f到2.0f范围内的（而不是原来的0.0f到1.0f）纹理坐标。
// 试试看能不能在箱子的角落放置4个笑脸
// 
// 纹理坐标的范围通常是从(0, 0)到(1, 1)，那如果我们把纹理坐标设置在范围之外会发生什么？
// OpenGL默认的行为是重复这个纹理图像（我们基本上忽略浮点纹理坐标的整数部分），通过纹理环绕方式来设定
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-11Textures_Exercise1", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
