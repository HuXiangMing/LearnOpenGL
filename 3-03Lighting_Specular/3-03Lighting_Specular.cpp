// 3-03Lighting_Specular : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
 
/*	
基础光照 镜面光照

冯氏光照中我们已经加了环境光和漫反射，我们就再把镜面高光(Specular Highlight)加进来，这样冯氏光照才算完整。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3-03Lighting_Specular", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
