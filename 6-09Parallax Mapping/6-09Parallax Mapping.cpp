// 6-09Parallax Mapping : 此文件包含 "main" 函数。程序执行将在此处开始并结束。

/*
视差贴图

应用：使用视差贴图，让一张2D图具有深度（高度）效果

前言：
OpenGL视差贴图是一种技术，它使用多个贴图来模拟立体视觉，从而使物体看起来更加立体。
它使用一个贴图来模拟物体的表面，另一个贴图来模拟物体的深度，从而使物体看起来更加立体。

视差贴图(Parallax Mapping)技术和法线贴图差不多，但它有着不同的原则。
和法线贴图一样视差贴图能够极大提升表面细节，使之具有深度感。
它也是利用了视错觉，然而对深度有着更好的表达，与法线贴图一起用能够产生难以置信的效果。
视差贴图和光照无关，我们只是作为法线贴图的技术延续来讨论它的。
...
视差贴图通常能够在复杂场景下较好地表达深度信息，适用于一些需要快速得到深度信息的应用。

视差贴图通常较为稳定，不容易受到光照和纹理等因素的影响。

缺点：

视差贴图通常只能提供相对粗糙的深度信息，对于一些需要精确深度信息的应用可能不够准确。

视差贴图的计算通常比陡峭视差贴图更慢，因为它需要进行归一化和平滑等操作，增加计算时间和计算复杂度。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "6-09Parallax Mapping", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
