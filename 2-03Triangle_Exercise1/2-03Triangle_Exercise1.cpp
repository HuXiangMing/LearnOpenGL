// 2-03Triangle_Exercise1 : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// 使用一组顶点数据（包含两个三角形的顶点数据）进行绘制两个三角形
...
int main()
{
    ...
    // glfw: 创建窗口
    // ------------------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-03Triangle_Exercise1", NULL, NULL);
    if (window == NULL)
    {
        printf("窗口创建失败");
