// 2-13Textures_Exercise3 : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// 
...
bool isAwesomefaceHorizontalReversal = false;//笑脸水平翻转
bool isAwesomefaceVerticalReversal = false;//笑脸垂直翻转

//三角形的顶点数据
float vertices[] = {
	// positions          // colors           // texture coords (注意，我们将它们更改为2.0f!)
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-13Textures_Exercise3", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
	...
	// glew加载完成后构建和编译着色器程序
	// ------------------------------------
	Shader ourShader = Shader("vertexSource.vert", "fragmentSource.frag");
...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//将片元着色器mixValue值固定在0-1之间浮动
		double  timeValue = glfwGetTime();
		float mixValue = static_cast<float>(sin(timeValue) / 2.0 + 0.5);
		ourShader.setFloat("mixValue", mixValue);
		ourShader.setBool("isAwesomefaceHorizontalReversal", isAwesomefaceHorizontalReversal);
		ourShader.setBool("isAwesomefaceVerticalReversal", isAwesomefaceVerticalReversal);
...
void processInput(GLFWwindow* window)
{
	...
	{
		isAwesomefaceHorizontalReversal = false;
	}
	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
	{
		isAwesomefaceHorizontalReversal = true;
	}

	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
	{
		isAwesomefaceVerticalReversal = true;
	}
	if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
	{
		isAwesomefaceVerticalReversal = false;
	}
}
