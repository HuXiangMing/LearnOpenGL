// 2-07Shader_Fragment_Interpolation : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// 片段插值，在顶点数据中添加定点色来给片元传递更多数据
...
int main()
{
    ...
    // glfw: 创建窗口
    // ------------------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2-07Shader_Fragment_Interpolation", NULL, NULL);
    if (window == NULL)
    {
        printf("窗口创建失败");
