// 6-11Parallax Mapping_Occlusion : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
前言：
与陡峭视差贴图（Disparity Map）和视差贴图（Depth Map）一样，都是计算机视觉中常用的图像处理技术
用于描述图像中不同位置的物体之间的距离关系。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "6-11Parallax Mapping_Occlusion", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
