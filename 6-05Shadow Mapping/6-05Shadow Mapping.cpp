// 6-05Shadow Mapping : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
条纹样式
虽然很多时候没问题，但是当光源以一个角度朝向表面的时候就会出问题，这种情况下深度贴图也是从一个角度下进行渲染的。
...
光照有一个区域，超出该区域就成为了阴影；这个区域实际上代表着深度贴图的大小，这个贴图投影到了地板上。
...
PCF（percentage-closer filtering）_降低阴影锯齿块
...
核心思想是从深度贴图中多次采样，每一次采样的纹理坐标都稍有不同。每个独立的样本可能在也可能不再阴影中。
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "6-05Shadow Mapping", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
...
#pragma region 帧缓冲配置
	...
	//光的视锥的投影坐标比1.0大的问题，将超出深度贴图的坐标的深度范围设置到1.0
