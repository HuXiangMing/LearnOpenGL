// 3-07Material_Map : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
 
/*
光照贴图
之前的材质系统是一个最简单的模型，我们希望通过某种方式对物体的每个片段单独设置漫反射颜色。引入漫反射和镜面光贴图(Map)。
这允许我们对物体的漫反射分量（以及间接地对环境光分量，它们几乎总是一样的）和镜面光分量有着更精确的控制。


漫反射贴图
是使用一张覆盖物体的图像，让我们能够逐片段索引其独立的颜色值，它是一个表现了物体所有的漫反射颜色的纹理图像。
在着色器中使用漫反射贴图的方法和纹理教程中是完全一样的。将纹理储存为Material结构体中的一个sampler2D
struct Material {
	sampler2D diffuse;
	vec3      specular;
	float     shininess;
...
可见，这里移除了环境光材质向量，因为环境光颜色在几乎所有情况下都等于漫反射颜色
但如果你非常估值，仍想将环境光设置为一个漫反射之外的值，那也可以保留原来的，但整个物体只能拥有一个环境光颜色

镜面光贴图
在有些时候，镜面高光看上去会有些奇怪，因为我们的物体大部分都是木头，木头不应该有这么强的镜面高光。
我们同样可以使用一个专门用于镜面高光的纹理贴图。让物体的某些部分以不同的强度显示镜面高光
...
int main()
{
	...
	// glfw: 创建窗口
	// ------------------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3-07Material_Map", NULL, NULL);
	if (window == NULL)
	{
		printf("窗口创建失败");
